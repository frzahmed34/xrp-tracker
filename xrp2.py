...
# ────────────────────────────
BINANCE_READONLY = "https://data-api.binance.vision"   # <— mirror
HEADERS = {"User-Agent": "Mozilla/5.0"}

def call_binance(endpoint: str, params: dict):
    r = requests.get(f"{BINANCE_READONLY}{endpoint}",
                     params=params, headers=HEADERS, timeout=10)
    # 200 = OK → return JSON
    if r.status_code == 200:
        return r.json()

    # anything else → show message & return empty
...
def get_klines(pair: str):
    data = call_binance("/api/v3/klines",
                        {"symbol": pair, "interval": "1d", "limit": 90})
    if not isinstance(data, list):
        return pd.DataFrame()          # failed ⇒ empty
...
df = get_klines(PAIR)
if df.empty:
    st.error("❌ Binance returned no rows (maybe still rate-limited). Try again later.")
    st.stop()
...
diff   = hi - lo
fib = {
    "0%": lo, "23.6%": hi-diff*0.236,
    "38.2%": hi-diff*0.382, "50%": hi-diff*0.5,
    "61.8%": hi-diff*0.618, "78.6%": hi-diff*0.786, "100%": hi,
}
st.subheader("Fib levels"); st.table(pd.DataFrame(fib.items(), columns=["Level","Price"]))

# ────────────────────────────
# Order-book (same mirror)
# ────────────────────────────
...
for p,_,v in top_b: st.write(f"🟢 ${p:,.2f} – {v:,.0f} USD")
st.subheader("Top 10 sell walls")
for p,_,v in top_a: st.write(f"🔴 ${p:,.2f} – {v:,.0f} USD")
